...
![搜索](/images/2024-02-29/Homebrew管理工具使用笔记-01.jpg)

2、进入github
![进入github](/images/2024-02-29/Homebrew管理工具使用笔记-02.jpg)
3、选择历史版本
![选择历史版本](/images/2024-02-29/Homebrew管理工具使用笔记-03.jpg)
![选择历史版本](/images/2024-02-29/Homebrew管理工具使用笔记-04.jpg)
4、下载历史版本安装代码
![下载历史版本安装代码](/images/2024-02-29/Homebrew管理工具使用笔记-04.jpg)
5、安装
...
&emsp;&emsp;说一个自己使用的想法，从使用mac系统就一直使用Homebrew。但是作为软件管理，总感觉对于历史版本问题很别扭。之前是保留多个版本可以按需切换，记得有一个```brew switch```命令，不知道啥时候没了，现在用```brew link```切换带版本号的软件包（形如：mysql@5.7），而不带版本的软件包（形如：nginx）一直不知道如何切换，之前都是手动修改软链，现在更是执行安装后就会把旧版本清理掉。记得之前是可以等一段时间不用才会被清理或者手动清理。而且默认一定时间之后，执行任何命令都会触发更新，已经不只一次因为安装一个软件包导致触发所有软件更新造成某些程序无法运行的问题，好在也都顺利解决了。
